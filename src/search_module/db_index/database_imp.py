...
class DatabaseImp(DatabaseInterface):
    ...
    def retrieve(self, query: List[int], atts: List[str], k: int) -> List[Tuple[int,float,str]]:
        ...
        if len(query) != vector_dim:
            raise Exception(f'Query vector should have dim 6. Found dim {len(query)}')
   
        for correct_att, query_att in zip( atts,  atts):
            if correct_att != query_att:
                raise Exception(f'Query attributes are in wrong order. They shoud be like this: {atts}.')
        ...
        result_set = [(id, dis, path) for  path, id, dis in zip(self.metadata['file'].iloc[list(ordered_ids)], ordered_ids, ordered_dist)]
